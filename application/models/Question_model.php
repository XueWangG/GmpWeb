...
class Question_model extends CI_Model {
	...
	public function get_attribute($type,$condition)
	{
		if($type==1){
			$this->db->select('category_id as id,category_id,category_name');
			$query = $this->db->get_where('category',$condition);
			$rs =  $query->row_array();
			return $rs;
		}else if($type==2){
			$this->db->select('difficulty_id as id,difficulty_id,difficulty_name');
			$query = $this->db->get_where('question_difficulty', $condition);
			$rs = $query->row_array();
			return $rs;
		}else if($type==3){
			$this->db->select('sop_id as id,sop_id,sop_name');
			$query = $this->db->get_where('question_sop',$condition);
			$rs =  $query->row_array();
			return $rs;
		}
	...
	public function  update_attribute($type,$id,$company_id,$data)
	{
		if($type==1){
			return $this->db->update('category',$data, array('category_id'=>$id,'company_id'=>$company_id));
		}else if($type==2){
			return $this->db->update('question_difficulty',$data, array('difficulty_id'=>$id,'company_id'=>$company_id));
		}else if($type==3){
			return $this->db->update('question_sop',$data, array('sop_id'=>$id,'company_id'=>$company_id));
		}
	
	}
	public function  delete_attribute($type,$id,$company_id)
	{
		if($type==1){
			return $this->db->update('category',array('del'=>1), array('category_id'=>$id,'company_id'=>$company_id));
		}else if($type==2){
			return $this->db->update('question_difficulty', array('del'=>1), array('difficulty_id'=>$id,'company_id'=>$company_id));
		}else if($type==3){
			return $this->db->update('question_sop', array('del'=>1), array('sop_id'=>$id,'company_id'=>$company_id));
		}
	...
	public function update_question($question_id,$company_id,$question_data,$choice_content,$choice_no)
	{
		$this->db->trans_start();
		$this->db->update('question',$question_data,array('question_id'=>$question_id, 'company_id'=>$company_id));
	...
	public function get_category_list($count=100)
	{
	...
	public function get_difficulty_list($count=100)
	{
	...
	public function get_question_detail($question_id)
	{
		...
		$this->db->join('question_sop', 'question_sop.sop_id = question.sop_id','left');
	...
	public function get_question_page($company_id,$type,$category_id,$cur_page,$page_size = 10)
	{		
		$result = array();
		...
		$this->db->where('question.company_id',$company_id);
		if($category_id!=0)
		{
	...
	public function delete_all($company_id,$ids)
	{
		$this->db->set('del', '1', FALSE);
		$this->db->where('company_id',$company_id);
		$this->db->where_in('question_id',explode('_', $ids));
		$this->db->update('question');
	}

	public function get_ready_for_testpaper($qtype, $cat,$sop,$testpaperId, $num=0, $offset=0)
	{
